...
def getLatestAncestorIndex(curVar, inducedAncestors, ordering):
	...
	for ancestor in inducedAncestors:
		if (ordering.index(curVar) > ordering.index(ancestor)):
			if latestAncestorIndex == None:
				latestAncestorIndex = ordering.index(ancestor)
			elif (ordering.index(ancestor) > latestAncestorIndex):
				latestAncestorIndex = ordering.index(ancestor)

	return latestAncestorIndex

def checkConsistencyCbj(curVal, curVar, toCheckVal, toCheckVar, constraints, ordering):
	for cons in constraints:
		# print curVar, " ", toCheckVar
		# print cons['scope']
		if not ((curVar in cons['scope']) and (toCheckVar in cons['scope'])):
			continue
...
def getValueCbj(curVar, ins, domains, constraints, ordering):
	...
	print "called getValueCbj for variable: ", curVar
	print "domain of: ",curVar," is: ",domain
	...
	while vi < len(domain):
		...
		for i in range(ordering.index(curVar)):
			...
			consistentFlagTemp, latestConstraint = checkConsistencyCbj(curVal, curVar, ins[ordering[i]], ordering[i], constraints, ordering)
			...
			if consistentFlagTemp:
				...
				print "value: ", curVal, " inconsistent with: ", ordering[i]
				consistentFlag = False
				scopesOfConflCons = (scopesOfConflCons | set(latestConstraint['scope'])) - set([curVar])
				break
...
if algoChoice == 2:
	...
	# varCons is dict {'X1':[constriant1, constraint2, constraint3...], 'X2':[constriant1, constraint2, constraint3...], ...}
	varConsCopy1 = varCons.copy()
	ancestors = dict()
	...
	# looping until we reach end of ordering array or empty domain for first variable in ordering
	while True:
		curVar = ordering[vi]
...
if algoChoice == 3:
	...
	while True:
		...
		curVal, jumpBackSetTemp = getValueCbj(curVar, solution, variablesCopy1, varConsCopy1[curVar], ordering)
		...
		if curVal == False:
			...
			latestAncestorIndex = getLatestAncestorIndex(curVar, list(jumpBackSet), ordering)
